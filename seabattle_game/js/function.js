(function () {
  ...
  class Ships {
    ...
    static showShip(self, shipname, x, y, kx) {
      ...
      div.style.cssText = `left:${y * Field.SHIP_SIDE}px; top:${
        x * Field.SHIP_SIDE
      }px;`;
  ...
  class Placement {
    ...
    onMouseMove(e) {
      ...
      if (
        ...
        const { x, y } = this.getCoordsCloneInMatrix({
          left,
          right,
          top,
          bottom,
  ...
  class Controller {
    ...
    init() {
      ...
      if (!isHandlerController) {
        ...
        computerfield.addEventListener(
          "contextmenu",
          this.setUselessCell.bind(this)
        );
      ...
      if (this.player === human) {
        ...
        compShot = true;
        this.text = "The computer shoots first";
        // выстрел компьютера
    ...
    isShipSunk() {
      ...
      let obj = Object.values(human.squadron).reduce((a, b) =>
        a.arrDecks.length > b.arrDecks.length ? a : b
      );
      // определяем, есть ли ещё корабли, с кол-вом палуб больше, чем попаданий
      if (
        this.tempShip.hits >= obj.arrDecks.length ||
    ...
    removeCoordsFromArrays(coords) {
      if (this.coordsAroundHit.length > 0) {
        this.coordsAroundHit = Controller.removeElementArray(
          this.coordsAroundHit,
          coords
        );
      }
      if (this.coordsFixedHit.length > 0) {
        this.coordsFixedHit = Controller.removeElementArray(
          this.coordsFixedHit,
          coords
        );
      }
      this.coordsRandomHit = Controller.removeElementArray(
        this.coordsRandomHit,
        coords
      );
    }
    ...
    showIcons(opponent, [x, y], iconClass) {
      ...
      function fn() {
        ...
        span.style.cssText = `left:${y * Field.SHIP_SIDE}px; top:${
          x * Field.SHIP_SIDE
        }px;`;
    ...
    makeShot(e) {
      ...
      switch (v) {
        ...
        case 4:
          Controller.showServiceText("You have already fired at these coordinates!");
          break;
      }
    }
    ...
    miss(x, y) {
      ...
      if (this.player === human) {
        text = "You missed. Shoots the computer.";
        this.player = computer;
        this.opponent = human;
        compShot = true;
        setTimeout(() => this.makeShot(), 2000);
      } else {
        text = "The computer failed. Your shot.";
    ...
    hit(x, y) {
      ...
      text =
        this.player === human
          ? "Congratulations! You hit. Your shot"
          : "The computer has entered your ship. Computer shot";
      setTimeout(() => Controller.showServiceText(text), 400);

      // перебираем корабли эскадры противника
      outerloop: for (let name in this.opponent.squadron) {
  ...
  buttonPlay.addEventListener("click", function (e) {
    ...
    toptext.innerHTML = "Naval battle between squadrons";
  ...
  buttonNewGame.addEventListener("click", function (e) {
    ...
    // очищаем поле игрока
    human.cleanField();
    toptext.innerHTML = "Arrangement of ships";
